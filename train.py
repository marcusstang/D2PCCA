...
def main():
    ...
    # Load previous ELBO data if the file exists
    try:
        with open(elbo_file_path, "rb") as f:
            elbo_data = pickle.load(f)
    except (FileNotFoundError, EOFError):
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        total_epoch_loss_train = train_KL_annealing(svi, train_loader, epoch, annealing_epochs=100,
                                                    minimum_annealing_factor=0.01, num_sectors=5)
        ...
        test_elbo.append(elbo2)
        print("[epoch %03d]  average test loss: %.4f" % (epoch, -elbo2))

        if epoch >= save_start_epoch:
            elbo_data.append((epoch, -elbo, -elbo2))  # Store epoch, train_elbo, and test_elbo
            with open(elbo_file_path, "wb") as f:
                pickle.dump(elbo_data, f)
